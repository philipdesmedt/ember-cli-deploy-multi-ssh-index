...
module.exports = CoreObject.extend({
  ...
  upload: function(options) {
    ...
    options['hosts'].forEach(function (host) {
      options['host'] = host;
      var promise = that.fetchRevisions(options).then(function(revisions) {
      ...
      plugin.log('✔  Finished with host ' + host + ' ' + promise, { verbose: true });
  ...
  activate: function(options) {
    ...
    var promisesArray = [];
    plugin.log('✔  Running revision activation...', { verbose: true });
    ...
    return new Promise(function(resolve, reject) {
      client.exec('cp ' + fromFile + ' ' + toFile, function(err) {
        if (err) {
          ...
          var str = '✔  Copied file ' + fromFile + ' to ' + toFile;
          plugin.log(str, { verbose: true });
          resolve();
        }
      });
    });
